...
import { Truck, Package, CheckCircle, Clock, XCircle, MapPin, CreditCard, ShoppingBag, ChevronRight, Printer, Download } from 'lucide-react';
...
const OrderTracking = () => {
  ...
  const breadcrumbItems = [
    ...
    ...(orderId ? [{ label: `Order ${orderId}`, path: `/orders/${orderId}` }] : [])
  ];
  ...
  useEffect(() => {
    const fetchOrder = () => {
      ...
      if (foundOrder) {
        ...
        const statuses = [
          { status: 'ordered', label: 'Order Placed', description: 'We have received your order', date: new Date(foundOrder.date), icon: ShoppingBag },
          { status: 'processing', label: 'Processing', description: 'Your order is being prepared', date: new Date(new Date(foundOrder.date).getTime() + 3600000), icon: Package },
          { status: 'shipped', label: 'Shipped', description: 'Your order is on the way', date: new Date(new Date(foundOrder.date).getTime() + 86400000), icon: Truck },
          { status: 'delivered', label: 'Delivered', description: 'Your order has been delivered', date: new Date(new Date(foundOrder.date).getTime() + 172800000), icon: CheckCircle }
        ];
        ...
        setTrackingData({
          currentStatus,
          estimatedDelivery: new Date(new Date(foundOrder.date).getTime() + 172800000),
      ...
      <>
        ...
        <div className="section-padding">
          <div className="container-custom text-center">
            ...
            <Link to="/shop" className="btn-primary mr-4">Continue Shopping</Link>
                        ...
                        {trackingData.statuses.map((step, index) => (
                          <div key={index} className="relative flex items-start">
                            <div className={`flex-shrink-0 w-12 h-12 rounded-full flex items-center justify-center z-10 ${
                              step.completed ? 'bg-gold text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-400'
                            }`}>
                            ...
                            <div className="ml-6 flex-1">
                              <div className={`font-semibold ${
                                ...
                                {step.label}
                                {step.current && (
                                  <span className="ml-2 text-sm bg-gold text-white px-2 py-1 rounded-full">Current</span>
                                )}
                              </div>
                ...
                <div className="flex items-center">
                  <div className="w-12 h-8 bg-gray-100 dark:bg-gray-700 rounded flex items-center justify-center mr-3">
                    {order.paymentMethod === 'creditCard' ? (
                      ...
                      <span className="font-bold">{order.paymentMethod === 'paypal' ? 'PP' : ''}</span>
                    )}
                  </div>
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      {order.paymentMethod === 'creditCard' ? 'Ending in 4242' : 'Connected'}
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div className="mt-12">
            <div className="bg-gradient-to-r from-gray-50 to-gold/5 dark:from-gray-800 dark:to-gold/10 rounded-2xl p-8">
              <div className="flex flex-col md:flex-row justify-between items-center">
                <div>
                  <h3 className="text-xl font-semibold mb-2">Questions about your order?</h3>
                  <p className="text-gray-600 dark:text-gray-300">
