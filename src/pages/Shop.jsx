...
import { Filter, Grid, List, ChevronDown, Plus, Minus, ShoppingCart, Star } from 'lucide-react';
...
const Shop = () => {
  ...
  const filteredProducts = allProducts.filter(product => {
    ...
    const inCategory = selectedCategory === 'All' || product.mainCategory === selectedCategory;
  ...
  return (
    <div className="py-12 ">
      <div className="container mx-auto px-4">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-4xl font-playfair font-bold text-gray-900 mb-2">Shop Our Collection</h1>
                ...
                <div className="space-y-3">
                  {categories.map(category => (
                    ...
                    >
                      ...
                      <span className={`${selectedCategory === category.name ? 'text-black' : 'text-gray-500'}`}>
                        ({category.count})
                      </span>
                    </button>
                  ))}
                ...
                <div className="flex items-center space-x-4 mb-4 md:mb-0">
                  <div className="flex items-center space-x-2">
                    <button
                      ...
                      className={`p-2 rounded-lg ${viewMode === 'grid' ? 'bg-gold text-white' : 'text-gray-600 hover:bg-gray-100'}`}
                    >
                      <Grid className="w-5 h-5" />
                    </button>
                    <button
                      onClick={() => setViewMode('list')}
                      className={`p-2 rounded-lg ${viewMode === 'list' ? 'bg-gold text-white' : 'text-gray-600 hover:bg-gray-100'}`}
                    >
